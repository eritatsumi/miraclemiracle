...
This might contribute to the small number density of small crater candidates.
...
We then propose a scaling law extended for cratering on coarse-grained targets (i.e., target grain size $\gtrsim$ projectile size).
...
We found that the shallow slope of the circular depressions on Itokawa cannot be accounted for by this new scaling law, suggesting that obliteration processes, such as regolith convection and migration, play a greater role for the depletion of circular depressions on Itokawa.
...
\end{align}
\end{comment}
The ratio of two terms changes as a function of $\pi_2^*$ and $\xi$. \red{More specifically, simply the new gravity scaling is applicable, when the energy ratio $\xi >>1$;
\begin{align}
...
%------------------------------------------------------------%
The cumulative number density of large circular depressions ($\gtrsim 100$ m) on Itokawa is comparable to the saturation crater density of the Moon.
...
\red{Thus, although the number of large craters are coincidentally at the saturation level of the Moon, the equivalent mechanism of erasure and production may not be the same as that on the Moon. Note that the actual surface age would not be much larger than our age due to the lack of any crater larger than 150 m.} 
