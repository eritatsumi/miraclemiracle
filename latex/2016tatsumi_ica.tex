...
with in the cosmic-ray exposure ages for returned samples from Itokawa which also reflect the age of 1 -- 10 m beneath the surface. 
...
after a possible catastrophic disruption of a rigid parent body of Itokawa suggested by Ar-Ar age ($\sim 1.3$ Gyr).
\end{abstract}
...
Japanese spacecraft Hayabusa made remote sensing observations of the near-Earth asteroid 25143 Itokawa on 2005 \citep{fujiwara2006, saito2006}. This was the first spacecraft observation of a rubble-pile asteroid and provided us with a variety of information on surface structures of a 
...
Subtracting the known masses of the largest asteroids \citep{demeo2014} from the total mass of the main belt, the minor asteroids ($<$20 km) would account for $\sim 35 \%$ of the total mass of MBAs. Another characteristic of small asteroids that can be observed by ground-based telescopes is the thermal 
inertia of asteroids, which allows us to estimate the surface physical conditions, such as grain size and inter-grain contact state. Typical km-sized asteroids 
...
This observation is consistent with the short collisional lifetimes of small asteroids. Such short surface age would prevent surface grains from grinding down to fine grains due to impact and/or thermal fatigue fragmentation.
...
Most of the surface is covered with cm to m-sized angular pebbles and boulders. The size range of boulders is rather large compared to Itokawa's small body ($0.54\times 0.29 \times 0.21$ km). More detailed description of boulder size distribution is given in \ref{boulder-dist}.
...
\red{First, we analyzed based on the classic $\pi$ scaling derived by \citet{holsapple1993} (Section \ref{sec:classic-pi}).
...
In order to understand this situation clearly, we use the ratio  $\xi =\frac{1}{2}m_pU^2/m_tQ_D^*$ of the impact energy to the disruption energy of a target grain as well as the size ratio $\psi=D_p/D_t$ of projectile to target grains and analyzed the relationship between the degree of armoring effect and experimental conditions for previous experiments. More specifically, we plotted the degree of armoring effect in the $\xi - \psi$ space (Figure \ref{phase}). Figure \ref{phase} clearly shows that the degree of armoring effect changes with both size ratio $\psi$ and energy ratio $\xi$. 
\red{More specifically, the experimental result of \citet{holsapple2014} exhibits that the conclusion of \citet{guettler2012} that the armoring effect occurs when $\psi<1$ is not always correct and impact energy might influence on the degree of armoring. }
...
\red{This figure shows that only the size ratio cannot describe the armoring condition and the energy ratio should be taken into account.
Thus, armoring condition should be examined in $\xi-\psi$ phase space.
...
\begin{figure}[htbp]
\begin{center}
	\includegraphics[width=80mm]{Figures/fig2.pdf}
...
The shapes of the projectiles are ``bullet shapes'' with spherical head and cylindrical tail and sphere for the former gun and for the latter gun, respectively.
...
By using weakly sintered-glass-beads \citep[e.g.][]{setoh2010} as target materials, we reproduced the condition of cratering involving disruption at relatively low impact velocities achievable with our facilities.
...
\begin{figure*}[tbhp]
	\centering
	\includegraphics[width=120mm]{Figures/fig3.eps}
...
\begin{figure}[htbp]
	\begin{center}
	\includegraphics[height=70mm]{Figures/fig4.eps}
...
\begin{figure}[htbp]
	\begin{center}
	\includegraphics[width=80mm]{Figures/fig6.eps}
...
\begin{figure*}[phbt]
	\begin{center}
	\includegraphics[width=100mm]{Figures/fig12.eps}
...
Cratering on fine-grain targets \citep{schmidt1980,mizutani1983,cintala1999} follows the gravity scaling for dry sand (solid line in Figure \ref{pi2-pebble}).
...
At lower impact velocities, however, our experimental data indicate that cratering efficiency is significantly reduced.
...
When the target grains are smaller than 1 mm, their crater sizes are almost the same as the fine grain targets which are several times larger than crater size on sand targets due to their small friction.
...
Thus, log $\pi_V$ should follow a straight line in $\pi_3$--$\pi_V$ space.
Furthermore, the $Y$-axis intersect of this line in the $\pi_3$--$\pi_V$ space would reveal the target material effective strength \citep[e.g.][]{holsapple1993}. 
...
\begin{figure}[htbp]
	\begin{center}
	\includegraphics[width=80mm]{Figures/fig7.pdf}
...
\begin{figure}[htbp]
	\begin{center}
	\includegraphics[width=80mm]{Figures/fig8.pdf}
...
\begin{figure}[htpb]
	\centering
	\includegraphics[width=80mm]{Figures/fig9.pdf}
...
The above experimental results suggest that the classic $\pi$ scalings (gravity or strength scaling) are not capable of describing accurately cratering on coarse-grained targets with armoring effect. This suggests that accurate reproduction of crater size of crater size on coarse-grained targets would require a new scaling law based on more in-depth consideration on cratering processes targets.
...
\begin{figure*}[phtb]
	\centering
	\includegraphics[width=120mm]{Figures/fig10.eps}
...
%\begin{figure}[htbp]
%	\begin{center}
%	\includegraphics[width= 100mm]{Figures/Chap2/xi_vs_aspect_ratio.eps}
%	\end{center}
...
However, that crater efficiency data smaller than the above $\pi^*$ values do not follow the gravity scaling trend (i.e., slope of $\sim -0.5$).
...
(1) No armoring effect (the classic gravity regime): $\psi>1$ or $\xi \gtrsim 1/\pi_2^*$ is sufficiently large, (2) armoring regime I, where the new crater scaling law can be applied to predict reduced size craters: $\psi<1$ and $1\lesssim\xi \lesssim1/\pi_2^*$, (3) armoring regime \II , where cater size is smaller than what our scaling law: $\psi<1$ and $\xi<1$.
...
\begin{figure*}[tbp]
	\centering
	\includegraphics[width=80mm]{Figures/fig13.pdf}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=80mm]{Figures/fig14.pdf}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=80mm]{Figures/fig15.eps}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=80mm]{Figures/fig16.pdf}
...
\begin{figure}
	\centering
	\includegraphics[width=80mm]{Figures/fig17.pdf}
...
%------------------------------------------------------------%
Because shockwaves travel at velocities much higher than subsequent excavation flow, in initial shock fragmentation due to collision between a projectile and target grains would complete before substantial excavation motion takes places.
...
The fashion that cratering efficiency is influenced by such momentum/energy transfer from the projectile to the target grains can be interpreted well when the degree of fragmentation of target grains is taken into account.
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=120mm]{Figures/fig18.eps}
...
Then we further discuss mass loss rate for small rubble-pile asteroids, such as Itokawa, Ryugu, and Bennu, and implications for long-term evolution of the asteroid belt.
...
We take the characteristic boulder size of 2 m because $P(>2 \text{m})\sim 0.5$ with the SFD condition of boulders on Itokawa (\ref{boulder-dist}), which means the probability for impactors to hit the surface boulders larger than 2 m is approximately 50 \% on Itokawa when $D_\text{min} \sim 0.1$.
...
If these five observed craters were formed in the near-Earth orbit, it would have taken more than 300 Myr due to much less encounters with other asteroids in the near-Earth orbit as we discussed above.
...
Consequently, it is highly likely that a large fraction of the large craters were formed in the main belt and this crater retention age may reflect the resurfacing  time of the asteroid in the main belt.
\begin{comment}
...
%------------------------------------------------------------%
Itokawa is the only body where the samples are returned other than the Moon and the only asteroid which has been investigated profoundly by both geochemical approach and remote sensing approach. Thus, it is very important to compare the geochemical ages with our crater retention age. Some of the ages exhibit values similar to our crater retention age, which reflect the resurfacing timescale for $\sim 10$ m of depth. It is not clear, however, what this coincidence actually means. In this section, we review the ages based on both samples and remote sensing data and discuss the implication of surface evolution of Itokawa.

For example, our crater retention age estimate (10 -- 33 Myr) based on the new scaling law for coarse grains is much younger than the previous age estimate (75 Myr -- 1 Gyr) based on the strength scalings for rigid bodies \citep{michel2009}. These different estimates can be examined with the results from the geochemical analyses of samples returned from Itokawa, such as spectral ages, cosmic ray exposure (CRE) ages, rim thickness ages, Ne deposition ages, and solar flare track distribution ages. Various ages of Itokawa are summarized in Figure as a function of characteristic depth. For example, the CRE ages, which reflect the residence time of several meters deep, were found $<$ 8 -- 66 Myr \citep{nagao2011} and $\sim$1.5 Myr for Ne (Meier+2014), and $>$3 -- 4 Myr for Be \citep{nishiizumi2015}, and the visible-near-infrared (VNIR) spectral ages, which reflect the exposure time of uppermost surface, were found $<$10 Myr \citep{koga2014} and ~2 -- 8 Myr \citep{bonal2015}.
 
First, it should be noted that the spectral ages and the CRE ages are estimated under the near-Earth condition where the solar wind flux is about 4 times that in the main belt. Thus, if the ages are estimated under the main belt condition, the ages would be up to about 4 times larger: the spectral ages are 2 -- 32 Myr \citep{bonal2015} and $<40$ Myr \citep{koga2014}, and the CRE ages are 3 -- 32 Myr \citep{nagao2011} and 1.5 -- 6 Myr \citep{meier2014}. Depending on the time ratio of the main belt and the near-Earth region, those ages could change, but the CRE ages and the spectral ages exhibit same trend to the time ratio. In contrast, crater age has opposite relationship to the time ratio of the main belt to the near-Earth region; i.e., crater retention age and boulder distribution age can be shorter and spectral age and CRE age can be longer as the residence time in the main belt is longer.

The CRE ages are the residence time for regolith particles in the top several meters of layer. The ages possibly reflect similar value to our crater retention ages for $\sim$10 m of depth. Moreover, the ``boulder exposure age'' is also an impact origin age, and is expected to have similar age with crater retention age. The boulder exposure age can be estimated from the boulder size distribution on the surface of Itokawa. \citet{basilevsky2014} estimated as 5 -- 75 Myr based on the impact disruption probability of meter size surface boulders. These ages are compatible with our crater retention ages, which also reflects the resurfacing age information of $\sim$10 m of depth. This will also help us cross-examine the accuracy of our crater scaling law on coarse-grained targets. Such agreement supports that the crater retention age for Itokawa obtained in this study is not much lower than the true.

Theoretical calculations have suggested that asteroids become smaller through both a catastrophic disruption of the parent body and subsequent impacts: collision cascade. Dynamical lifetimes are shorter for smaller asteroids. The crater resurfacing timescale for the upper $\sim$10 m layer, which is estimated from craters $>$ 100 m has important implications for collisional history of Itokawa. The fact that the crater retention age estimated based on a strength scaling by Michel+2009 overlaps with the Ar degassing age by \citet{park2015}. \citet{park2015} may suggest that the crater retention age would reflect the catastrophic disruption of the parent body. However, our crater retention age based on the new scaling law for coarse-grained targets, which is more realistic conditions for the rubble pile asteroid Itokawa, may reflect a more recent event, such as a subsequent disruption or global resurfacing, on Itokawa in the main belt after catastrophic disruption of a rigid parent body. Such subsequent disruption and/or global resurfacing event after the initial breakup from the parent body is not surprising, considering its short collisional lifetime due to its small size \citep[e.g.,][]{obrien2005}.

Apparently, there is a large gap among the upper most layer ages, the spectral ages $\sim$1 -- 10 Myr and the sample age based on rim thickness and Ne deposition $\sim$ 100 -- 1000 yr. Here, we will discuss what these ages may indicate.

There are two spectral ages; one is the average spectral age from remote sensing data \citep{koga2014} and another is the individual spectral age from the returned samples \citep{bonal2015}. Their ages, $<$ 10 Myr and $\sim$ 2 -- 8 Myr, are consistent each other. However, the spectral variation of ordinary chondritic material is estimated based on experiments by \citet{strazzulla2005}, which consider only Ar in the solar wind. More resent study, \citet{loeffler2009}, showed that He ions could be more effective for space weathering because of large flux and make the space weathering of Olivine sample ($<$ 45 um) saturated in $\sim$ 13,000 yr which is much faster than the time scale of space weathering by Ar ions $\sim$ 1.3 Myr. Because the spectra of both Itokawa particles and remote sensing data has not been saturated yet, the exposure time of upper most grains, i.e., microscopic exposure age, would be 10 times shorter when the effective He ion irradiation is taken into account. On the other hand, using the same method as \citet{bonal2015,brunetto2006b} estimated the spectral age of Karin family $\sim$ 2 Myr. This age is consistent with the possible disruption age of the parent body of Karin family estimated based on the orbital distribution and the N-body simulation by \citet{nesvorny2002}. The ages estimated based on the heavy ion irradiation experiment \citep{strazzulla2005} would be apparent ages including partial rejuvenate by granular convection or regolith disruption, i.e., macroscopic exposure ages. Thus, the spectral ages of Itokawa $\sim$ 1--10 Myr \citep{koga2014,bonal2015} which are estimated based on the experimental result by \citet{strazzulla2005}, may also indicate the global apparent age including rejuvenate.

Other uppermost surface ages have also been estimated based on solar wind damaged rim thickness and Ne deposition of sample particles. The solar wind damaged rim thickness of olivine is expected to be saturated in $\sim$ 10,000 yr reaching to 100 nm thick \citep{keller2016}. However, the ablated rim thickness of Itoakwa samples is $\sim$ 60-70 nm. Thus, the exposure ages of samples are several thousand years \citep{berger2015}. For the Ne deposition ages, the time spans of samples are 150, 410, and 550 years ignoring the erosion of grain surfaces and backscatter effect of Ne ions. Thus, they give lower limits; the actual irradiation ages might be several time longer, i.e., less than several thousand years \citep{nagao2011}.

Thus, these ages of uppermost layer show the different timescales of Itokawa surface layer, 1--10 Myr and 100 -- 1000 yr. They possibly indicate the macroscopic age and the microscopic age.
\begin{itemize}
\item
The microscopic age (100 -- 1000 yr) is the exposure age of upper most layer samples. This short exposure age indicates the high mobility of regolith of Itokawa, for example migration and convection of regolith \citep{miyamoto2007}. Moreover, this sample ages indicate that the timescale of space weathering for individual particles would be much shorter than it is expected.
\item
The macroscopic age (1-- 10 Myr) is the apparent exposure age including complex processes of regolith, such as regolith gardening, granular convection, and regolith fragmentation. Because our crater retention age is also macroscopic age, it is more appropriate to compare directly with this age. The coincidence of the spectral age and our crater retention age indicates that the global resurface could not be occurred for $\sim$ 10 Myr.
\end{itemize}

In order to confirm this hypothetical discussion, more analyses about the ``apparent age'' are needed. Finally, those analyses and interpretations of the surface processes of small asteroids are also meaningful for the landing site selection of Hayabusa2 and OSIRIS-REx.

\begin{figure}[htbp]
	\begin{center}
	\includegraphics[width=80mm]{Figures/fig19_new.pdf}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=120mm]{Figures/fig20.eps}
	\caption{Comparison among Itokawa surface ages estimated from the analyses of returned samples and the remote-sensing data. The solid lines indicate original literature values based on near-earth orbits. Dashed-lines indicate ages recalculated with a reduced Solar wind flux (1/4 that at the current near-Earth orbit) in the main belt \citep{obrien2005, michel2009, nagao2011, basilevsky2014, koga2014, meier2014, noguchi2014, berger2015, bonal2015, nishiizumi2015, park2015}.}
	\label{itokawa-ages}
	\centering
\end{figure}
...
%------------------------------------------------------------%
The size frequency distribution (SFD) is an important factor that controls \red{mass flux induced by} the radial drift of asteroids through Yarkovsky effect.
...
The new scaling law for coarse-grained targets allows us to predict that five 100-m diameter craters on Itokawa may have been made over 10 -- 33 Myr of time, which is still much younger than the previous estimated age (75 Myr -- 1 Gyr) based on strength scalings \citep{michel2009}.
...
The CRE ages of from several to 10 Myr which would reflect the age for the top several m of thickness \citep{nagao2011, meier2014, nishiizumi2015} are compatible with our crater retention ages which reflects the age information of $\sim 10$ m deep.
...
\begin{figure}[htbp]
	\begin{center}
	\includegraphics[width=7cm]{Figures/fig22.eps}
